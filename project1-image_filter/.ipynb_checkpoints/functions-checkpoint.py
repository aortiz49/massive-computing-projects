...
def init_globalimage(img, filt, sol):
    global image
    global my_filter
...
def image_filter(image,filter,numprocessors,filtered_image):
    ...
    # Now we will make define an object function that contains the filter that will be used in the pool function according to the dim of the filter: func.chosenfilter
    
    if size[0] < size [1]:  # row filter 
        func.chosenfilter = rowed_filter()
    elif size[0]==size[1]: # square filter
        func.chosenfilter = squared_filter()
    else:                  # column filter
        func.chosenfilter = columned_filter() 
    
    # Once this function object is fulfilled, we just need to initiate the pool itself: 
    
    # We stablish our shared_space as the filtered_image input given for this purpose and choose the number of processors: 
        
    rows=range(image.shape[0])
    with mp.Pool(processes=numprocessors,initializer=my.pool_init,initargs=[filtered_image,image,my_filter]) as p:
        p.map(func.chosenfilter,rows)
...
def columned_filter(r): 
    ...
    if (my_filter.shape[0]==5):
        ...
        # for each pixel in the row
        for i in range(cols - 1):
            res_row[i, d] = int((pp_row[i, d]*my_fylter[4] + p_row[i, d]*my_fylter[3] + c_row[i, d]*my_fylter[2] + n_row[i, d]*my_fylter[1]+nn_row[i, ]*my_fylter[0])) 
